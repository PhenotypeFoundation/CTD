...
return-1;};}};init_tables=function(){var i,element,table_type,tables_nodeList,contain_nested;top_position=0;tables_nodeList=div_drag.getElementsByTagName('table');for(i=0;i<tables_nodeList.length;i++){element=tables_nodeList[i].parentNode;do{if(element===div_drag){contain_nested=tables_nodeList[i].getElementsByTagName('table');if(contain_nested.length===0){table_type=2;top_position++;}
else{table_type=0;}
break;}
if(element.nodeName==='TD'){table_type=1;element.table=true;top_position++;break;}
element=element.parentNode;}while(element);tables[i]=tables_nodeList[i];tables[i].redips_tableType=table_type;tables[i].redips_idx=i;}
...
else if(e.redips_tableType===0&&idx!==top_position){tmp=tables[top_position];tables[top_position]=tables[idx];tables[idx]=tmp;}};handler_onmouseup=function(e){var evt=e||window.event,target_table,X,Y,i,target_elements,target_elements_length;X=evt.clientX;Y=evt.clientY;edge.flag.x=edge.flag.y=0;if(obj.releaseCapture){obj.releaseCapture();}
...
table_old=row_old=cell_old=null;};handler_onmousemove=function(e){var evt=e||window.event,bound=REDIPS.drag.bound,sca,X,Y,i,scrollPosition;X=pointer.x=evt.clientX;Y=pointer.y=evt.clientY;if(moved_flag===0&&(obj.className.indexOf('clone')>-1||(REDIPS.drag.clone_shiftKey===true&&evt.shiftKey))){clone_object();cloned_flag=1;REDIPS.drag.myhandler_cloned();set_color();}
else if(moved_flag===0){if(obj.setCapture){obj.setCapture();}
obj.style.position='fixed';calculate_cells();set_trc();set_color();REDIPS.drag.myhandler_moved();}
...
set_color();}};handler_onresize=function(){if(typeof(window.innerWidth)==='number'){window_width=window.innerWidth;window_height=window.innerHeight;}
